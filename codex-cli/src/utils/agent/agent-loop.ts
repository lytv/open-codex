...
export class AgentLoop {
  ...
  private getTools(): Array<{
    ...
    const defaultTools = [
      {
        ...
        function: {
          name: "shell",
          description: "Runs a shell command, and returns its output. Use this tool for file operations (apply_patch, pre-commit, git status, etc).",
          strict: false,
          parameters: {
            type: "object",
            properties: {
    ...
    if (this.mcpClient && this.mcpClient.isEnabled()) {
      try {
        ...
        // Lọc bỏ functions.shell nếu đã có shell mặc định để tránh trùng lặp
        const filteredMcpTools = mcpTools.filter(tool => 
          tool.function.name !== "functions.shell" &&
          tool.function.name !== "shell"
        );
        
        if (filteredMcpTools.length > 0) {
          if (isLoggingEnabled()) {
            log(`Adding ${filteredMcpTools.length} MCP tools to the request`);
          }
          return [...defaultTools, ...filteredMcpTools];
        }
      } catch (error) {
        if (isLoggingEnabled()) {
  ...
  private async handleFunctionCall(
    ...
    if (this.mcpClient && this.mcpClient.isEnabled() && name) {
      // Kiểm tra xem có phải là shell tool hay không (hỗ trợ cả functions.shell và shell)
      if (name === "shell" || name === "functions.shell") {
        if (isLoggingEnabled()) {
          log(`Executing shell command via ${name === "shell" ? "built-in" : "MCP"} tool`);
        }
        
        // Gọi handleExecCommand cho lệnh shell
        const {
          outputText,
          metadata,
      ...
      // Xử lý các MCP tool khác nếu có dạng serverName.toolName
